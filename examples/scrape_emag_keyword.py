...
class ItemCardParseResult:
    pnk: str
    top_favorite: bool  # 是否带有 top 标志
    image_url: Optional[str]  # 产品图 url
...
class KeywordSearchResults:
    keyword: str
    products: list[ItemCardParseResult]
...
async def start_scrape(cwd: Path, target_rows: Sequence[int]):
    ...
    try:
        for row in target_rows:
            ...
            # 遍历爬取结果，插入到工作表中
            for row_result, product_result in enumerate(one_keyword_result.products, start=2):
                # 设置该行的高度
                one_keyword_result_sheet.row_dimensions[row_result].height = 40

                # 产品详情页 url
                one_keyword_result_sheet.cell(row=row_result, column=1, value=product_result.url)
                one_keyword_result_sheet.cell(row=row_result, column=1).alignment = align

                # 产品图
                if product_result.image_url is not None:
                    # TODO 插入图片
                    one_keyword_result_sheet.cell(row=row_result, column=2, value=product_result.origin_image_url)
                    one_keyword_result_sheet.cell(row=row_result, column=2).alignment = align
                    one_keyword_result_sheet.cell(row=row_result, column=2).hyperlink = Hyperlink(
                        ref=product_result.origin_image_url,
                        target=product_result.origin_image_url,
                    )
                    one_keyword_result_sheet.cell(row=row_result, column=2).font = hyperlink_font
                    one_keyword_result_sheet.cell(row=row_result, column=2)

                # Top Favorite
                one_keyword_result_sheet.cell(row=row_result, column=3, value=product_result.top_favorite)
                one_keyword_result_sheet.cell(row=row_result, column=3).alignment = align

                # 评论数
                if product_result.review_count is not None:
                    one_keyword_result_sheet.cell(row=row_result, column=4, value=product_result.review_count)
                    one_keyword_result_sheet.cell(row=row_result, column=4).alignment = align

                # 价格
                if product_result.price is not None:
                    one_keyword_result_sheet.cell(row=row_result, column=5, value=product_result.price)
                    one_keyword_result_sheet.cell(row=row_result, column=5).alignment = align
...
async def download_product_image(cwd: Path, url: str, pnk: str, client: ClientSession, fake_header: dict[str, str]):
    ...
    async with client.get(
        ...
        try:
            ...
            await write_bytes_async(file=cwd.joinpath(f'temp/emag_product_images/{pnk}.{img_format}'))
...
if __name__ == '__main__':
    ...
    async def main():
        ...
        abort_res: set[RT] = (RT.MEDIA, RT.IMAGE, RT.STYLESHEET, RT.FONT)
